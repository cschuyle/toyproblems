...
(defn crossproduct
  ...
  (->> (for [a set-a
             b set-b]
         (operator a b))
       (remove nil?) ;; get rid of nils caused by div-by-0
       (into #{})))
...
(def order
  "Return 'order(n)', the set of values generated by all expressions
  containing n nines.  Memoized for your pleasure."
  (memoize
   (fn [n]
     (if (= 1 n)
       #{9}
